...
func ParseQueryString(_query string) (url string, args [][]string) {
	...
	url = os.Expand(url, func(env_var string) string {
		retrieved_env_var := os.Getenv(env_var)
		if retrieved_env_var == "" {
			fmt.Printf("Unable to get environment variable for API Access Key: %v", env_var)
			return ""
		}
		return retrieved_env_var
